// RH_RF95.cpp
// mods for Feather M0 sleep and wakeup upon packet reception by SJR
//
...
bool RH_RF95::setupInterruptHandler()
{
	...
	if (_myInterruptIndex == 0)
	    LowPower.attachInterruptWakeup(interruptNumber, isr0, RISING);  //mod by SJR 5/4/2024
	else if (_myInterruptIndex == 1)
	    LowPower.attachInterruptWakeup(interruptNumber, isr1, RISING);  //mod by SJR 5/4/2024
	else if (_myInterruptIndex == 2)
	    LowPower.attachInterruptWakeup(interruptNumber, isr2, RISING);  //mod by SJR 5/4/2024
	else
	    return false; // Too many devices, not enough interrupt vectors
    }
    return true;
