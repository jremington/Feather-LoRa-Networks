...
void setup()
{
  ...
  */
  ...
  manager.addRouteTo(REPEATER2_ADDRESS, REPEATER2_ADDRESS);
  manager.addRouteTo(CLIENT3_ADDRESS, REPEATER2_ADDRESS);

  // check repeater connectivity
  
  Serial.print("Check repeater connection: ");
  char test[] = "Test msg to repeater";
  if (manager.sendtoWait((uint8_t *)test, sizeof(test), REPEATER2_ADDRESS) == RH_ROUTER_ERROR_NONE)
  {
    ...
    {
      Serial.print("Reply from repeater node#");
      Serial.print(from);
      Serial.print(": ");
    ...
    {
      Serial.println("No reply from repeater");
    }
  }
  else
    Serial.println("sendtoWait failed. Is the repeater running?");
...
void loop()
{
  ...
  if (message_to_send) {
    ...
    if (manager.sendtoWait((uint8_t *)message_buffer, strlen(message_buffer) + 1, CLIENT3_ADDRESS) != RH_ROUTER_ERROR_NONE)
