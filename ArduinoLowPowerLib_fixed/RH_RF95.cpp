...
#include <RH_RF95.h>
//#include <ArduinoLowPower.h>  //mod by SJR 5/4/2024
...
bool RH_RF95::setupInterruptHandler()
{
	...
	if (_myInterruptIndex == 0)
	    attachInterrupt(interruptNumber, isr0, RISING);  //mod by SJR 5/4/2024
	else if (_myInterruptIndex == 1)
	    attachInterrupt(interruptNumber, isr1, RISING);  //mod by SJR 5/4/2024
	else if (_myInterruptIndex == 2)
	    attachInterrupt(interruptNumber, isr2, RISING);  //mod by SJR 5/4/2024
	else
	    return false; // Too many devices, not enough interrupt vectors
    }
    return true;
...
//    Serial.println(_enableCRC, HEX);
 ...
 //   spiWrite(RH_RF95_REG_12_IRQ_FLAGS, 0xff); // Clear all IRQ flags  //re-enabled SJR 5/4/2024 (does not seem to help)
...
//    LowPower.deepSleep();  //mod by SJR 5/4/2024
	}
}
