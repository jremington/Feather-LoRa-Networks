...
//#define RH_TEST_NETWORK 1  // if defined in RHRouter.h => limited connectivity test network 1-2-3-4
...
#define THIS_NODE 1  //white antenna COM12

uint8_t nodeId;
uint8_t routes[N_NODES]; // route status for report, updated frequently
int16_t rssi[N_NODES]; // signal strength info
uint16_t packet_num[N_NODES] = {0}; //optional packet ID included in message content, incremented for each node separately
...
uint32_t network_info_report = 30000UL; //update network info every 30 seconds
uint32_t last_network_info_report = 0; //last time reported
...
void loop() {
  ...
  if (n != nodeId) { // not self
    ...
    snprintf(buf, sizeof (buf), "#%u: %lu mV", packet_num[n-1]++, vbat);
    ...
    /*
        //log ACK/NACK and/or sent message to console
        Serial.print(">");
        Serial.print(n);
    ...
    */
    ...
    // time for network info report?

    if (millis() - last_network_info_report > network_info_report) {
      last_network_info_report = millis();
    ...
    //blocking call with variable timeout, determines most of the loop time.
    ...
    if (manager->recvfromAckTimeout((uint8_t *)buf, &len, waitTime, &from, NULL, NULL, &flags, &hops))
    {
      ...
      if (route != NULL ) { //fixed null pointer handling
        via = route->next_hop;
