// modified from rf22_router_server1.pde
// -*- mode: C++ -*-
...
//server1 2 yellow antenna COM14
//server2 3 red antenna COM9
//server3 4 black antenna COM11
...
void setup()
{
  ...
  if (!manager.init()) {
    ...
    Serial.println("Server 1 Init OK");
  }
  ...
  */
  ...
  manager.addRouteTo(SERVER3_ADDRESS, SERVER2_ADDRESS); //server2 is the next hop to server3

  // check neighbor node connectivity

  char test[] = "message to server3";
  if (manager.sendtoWait((uint8_t *)test, sizeof(test), SERVER3_ADDRESS) == RH_ROUTER_ERROR_NONE)
  {
    ...
    {
      Serial.println("No reply from server3");
    }
  }
  else
