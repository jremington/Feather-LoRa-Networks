//Self-organizing standalone RHMesh network for Feather LoRa modules
// ATmega32U4 version. Not enough RAM for OLED or encryption, tested and works with Feather M0 units!

// ping message type added to update network connectivity
// text message capability added on Serial1
// "e" -> AES128 encryption for text messages added 4/17/2024. Works well on M0
//
...
//#define FEATHER_OLED  //for FeatherWing OLED display. Libs require too much memory for ATmega32U4
//#define ENCRYPT

#define N_NODES 2  //max 10 given RHRouter table dimensions
#define THIS_NODE 1  //green antenna COM28 (OLED) *** hard coded node address *** (no EEPROM)
...
#define RCV_MSG_OUTPUT_OLED

#define LOG_NETWORK_INFO   //print routing and rssi report on above
//#define LOG_ACK_NACK       //print response to sent messages
...
#ifdef ENCRYPT
AES128 aes128;
RHEncryptedDriver driver(rf95, aes128); // Instantiate the driver with those two
...
char report[50];  //network node report
...
void setup() {
  ...
  // set up manager for encrypted (or not) driver
  ...
  if (!manager->init()) {
    Serial.println(F("Radio manager init failed"));
  ...
  */
  ...
  display.println(F("FeatherMesh init OK"));
  display.print(F("Node ")); display.print(nodeId);
  display.print(" "); display.print(TxPower); display.print(F("dBm @ "));
...
void loop() {
  ...
  if (n != nodeId) { // not self
    ...
    if (message_to_send and message_target_node == n) { //this node is the target
      ...
      manager->setRetries(2); //two retries for ping messages
...
#ifdef FEATHER_OLED
#ifdef RCV_MSG_OUTPUT_OLED
      // log ASCII message received to display
...
// translate error number to message. The string constants below don't seem to placed in RAM. Shortening has no effect.
